...
from notebook.settings import *
from notebook.functions import (get_path, get_breadcrumbs,
    get_filterdate, get_settings_var, get_directory, convert_filename)
from notebook.templatetags.fb_tags import query_helper
from notebook.base import FileObject
from notebook.decorators import flash_login_required

from mezzanine.utils.importing import import_dotted_path

from storages.backends.s3boto import S3BotoStorage
default_storage = S3BotoStorage(bucket=AWS_STORAGE_BUCKET_NAME)

# Add some required methods to FileSystemStorage
storage_class_name = django_settings.DEFAULT_FILE_STORAGE.split(".")[-1]
mixin_class_name = "notebook.storage.%sMixin" % storage_class_name

# Workaround for django-s3-folder-storage
if django_settings.DEFAULT_FILE_STORAGE == 's3_folder_storage.s3.DefaultStorage':
    mixin_class_name = 'notebook.storage.S3BotoStorageMixin'
...
def browse(request):
    ...
    if path is None:
        ...
        redirect_url = reverse("nb_browse") + query_helper(query, "", "dir")
    ...
    return render_to_response('notebrowser/index.html', {
        'dir': path,
        'p': p,
        'page': page,
...
def mkdir(request):
    ...
    from notebook.forms import MakeDirForm
    ...
    if path is None:
        msg = _('The requested Folder does not exist.')
        messages.add_message(request, messages.ERROR, msg)
        return HttpResponseRedirect(reverse("nb_browse"))
    abs_path = os.path.join(get_directory(), path)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            try:
                ...
                # remove pagination
                redirect_url = reverse("nb_browse") + query_helper(query, "ot=desc,o=date", "ot,o,filter_type,filter_date,q,p")
                return HttpResponseRedirect(redirect_url)
    ...
    return render_to_response('notebrowser/makedir.html', {
        'form': form,
        'query': query,
...
def upload(request):
    ...
    return render_to_response('notebrowser/upload.html', {
        'query': query,
...
def create(request):
    ...
    return render_to_response('notebrowser/notebook.html', {
        'query': query,
        'title': _(u'Select files to upload'),
        'settings_var': get_settings_var(),
        'session_key': session_key,
...
def edit(request):
    ...
    tripped = filepath[filepath.index('/')+1:]
    return redirect(NOTEBOOK_URL + '/user/' + request.user.username + '/notebooks/' + tripped)
...
def _check_file(request):
    ...
    nb_uploadurl_re = re.compile(r'^.*(%s)' % reverse("nb_upload"))
    folder = nb_uploadurl_re.sub('', folder)
    fileArray = {}
    if request.method == 'POST':
...
def _upload_file(request):
    ...
    if request.method == 'POST':
        ...
        if get_params:
            return HttpResponseRedirect(reverse('nb_browse') + get_params)
...
def delete(request):
    ...
    return HttpResponseRedirect(reverse("nb_browse") + qs)
...
def rename(request):
    ...
    from notebook.forms import RenameForm
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            try:
                ...
                messages.add_message(request, messages.SUCCESS, msg)
                redirect_url = reverse("nb_browse") + query_helper(query, "", "filename")
                return HttpResponseRedirect(redirect_url)
    ...
    return render_to_response('notebrowser/rename.html', {
        'form': form,
        'query': query,
